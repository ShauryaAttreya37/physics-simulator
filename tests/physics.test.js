...
describe('Double Pendulum Conservation', () => {
  ...
  function derivs(state, p) {
    ...
    const dom1 = (-p.g * (2 * p.m1 + p.m2) * Math.sin(th1) - p.m2 * p.g * Math.sin(th1 - 2 * th2) - 2 * Math.sin(d) * p.m2 * (om2 * om2 * p.l2 + om1 * om1 * p.l1 * cosD)) / (p.l1 * den);
    const dom2 = (2 * Math.sin(d) * (om1 * om1 * p.l1 * (p.m1 + p.m2) + p.g * (p.m1 + p.m2) * Math.cos(th1) + om2 * om2 * p.l2 * p.m2 * cosD)) / (p.l2 * den);
  ...
  function energy(state, p) {
    ...
    const T = 0.5 * (p.m1 + p.m2) * p.l1 * p.l1 * om1 * om1 + 0.5 * p.m2 * p.l2 * p.l2 * om2 * om2 + p.m2 * p.l1 * p.l2 * om1 * om2 * cosD;
...
describe('Orbital Gravity (N-Body) Conservation', () => {
  ...
  it('Yoshida4 should conserve energy in figure-8 orbit', () => {
    ...
    const q = [[-0.97000436, 0.24308753], [0.97000436, -0.24308753], [0, 0]];
    const v = [
