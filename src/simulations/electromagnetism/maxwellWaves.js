...
export function create(canvas, initParams = {}) {
  ...
  function render() {
    ...
    ctx.beginPath(); ctx.moveTo(zStart.px, zStart.py); ctx.lineTo(zEnd.px, zEnd.py); ctx.stroke();
    
    const gridSize = 10;
    ctx.strokeStyle = 'rgba(34, 211, 238, 0.04)';
    ctx.beginPath();
    for(let z=0; z<=gridSize; z++) {
        let p1 = project(-1.5, 0, z); let p2 = project(1.5, 0, z);
        ctx.moveTo(p1.px, p1.py); ctx.lineTo(p2.px, p2.py);
    }
    for(let x=-1.5; x<=1.5; x+=0.5) {
        let p1 = project(x, 0, 0); let p2 = project(x, 0, gridSize);
        ctx.moveTo(p1.px, p1.py); ctx.lineTo(p2.px, p2.py);
    }
    ctx.stroke();
    ...
    ctx.beginPath();
    for(let z=0; z<=gridSize; z++) {
        let p1 = project(0, -1.5, z); let p2 = project(0, 1.5, z);
        ctx.moveTo(p1.px, p1.py); ctx.lineTo(p2.px, p2.py);
    }
    for(let y=-1.5; y<=1.5; y+=0.5) {
        let p1 = project(0, y, 0); let p2 = project(0, y, gridSize);
        ctx.moveTo(p1.px, p1.py); ctx.lineTo(p2.px, p2.py);
    }
    ctx.stroke();
    ...
    const renderWave = (pts, offset, eColor, bColor, strokeW, isCore) => {
      ...
      for (let i = 0; i <= pts; i++) {
          ...
          const b = p.amplitude * env * Math.cos(phase + offset*2) * Math.exp(-offset*offset*20);
          const pt = project(0, b + offset*0.5, z);
          if (i===0) ctx.moveTo(pt.px, pt.py); else ctx.lineTo(pt.px, pt.py);
      }
      ctx.strokeStyle = bColor;
      ctx.lineWidth = strokeW;
      if (isCore) { ctx.shadowBlur = 15; ctx.shadowColor = '#10b981'; }
      ctx.stroke();
      ...
      for (let i = 0; i <= pts; i++) {
          ...
          const env = Math.min(1, z * 2);
          const phase = k * z - omega * simTime;
          const e = p.amplitude * env * Math.cos(phase + offset*2) * Math.exp(-offset*offset*20);
          const pt = project(e + offset*0.5, 0, z);
          if (i===0) ctx.moveTo(pt.px, pt.py); else ctx.lineTo(pt.px, pt.py);
      }
      ctx.strokeStyle = eColor;
      ctx.lineWidth = strokeW;
      if (isCore) { ctx.shadowBlur = 15; ctx.shadowColor = '#06b6d4'; }
      ctx.stroke();
      if (isCore) ctx.shadowBlur = 0;
    };
    ...
    for (let s = 0; s < strands; s++) {
      ...
      renderWave(points, strandOffset, `rgba(34, 211, 238, ${alpha})`, `rgba(52, 211, 153, ${alpha})`, 1.5, false);
    }
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        if (env > 0.01) {
            ...
            ctx.beginPath(); ctx.arc(pE.px, pE.py, 2, 0, Math.PI*2); ctx.fill();
            
            const pB = project(0, b, zPos);
            ctx.beginPath(); ctx.arc(pB.px, pB.py, 2, 0, Math.PI*2); ctx.fill();
        }
    }
    ...
    ctx.beginPath(); ctx.moveTo(oPtNegE.px, oPtNegE.py); ctx.lineTo(oPtE.px, oPtE.py); ctx.stroke();
    ctx.shadowBlur = 0;
    
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.arc(oPtE.px, oPtE.py, 4, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(oPtNegE.px, oPtNegE.py, 4, 0, Math.PI*2); ctx.fill();

    // ── HUD Text ────────────────────────────────────────────────
    ctx.font = 'bold 12px "Inter", "JetBrains Mono", sans-serif';
    ctx.shadowBlur = 4;
    ctx.shadowColor = 'rgba(0,0,0,0.8)';
    ctx.fillStyle = '#22d3ee'; ctx.fillText('Electric Field (E)', 20, 30);
    ctx.fillStyle = '#34d399'; ctx.fillText('Magnetic Field (B)', 20, 50);
    ctx.shadowBlur = 0;
  ...
  return {
    ...
    reset() { this.stop(); simTime = 0; render(); this.start(); },
    setParams(next) {
      p = { ...p, ...next };
      render();
    },
