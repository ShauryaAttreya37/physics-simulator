...
function derivs(th, ph, ps, th_v, ph_v, ps_v, p) {
  ...
  const dth_v = ph_v * ph_v * sin_th * cos_th 
...
export const scenarios = [
  {
    ...
    description: 'High initial spin avoids nutation wobble, causing a smooth slow precession, identical to the classic bicycle wheel demo.',
    params: { startOmegaPsi: 150, startOmegaPhi: 0, tiltAngle: 1.2 },
  },
  {
    name: 'Slow Spin (Nutation Petals)',
    description: 'When released from rest with low spin, the top falls due to gravity, creating a visible bobbing motion called nutation.',
    params: { startOmegaPsi: 20, startOmegaPhi: 0, tiltAngle: 1.0, damping: 0.002, spinDecay: 0 },
  },
  {
    name: 'Sleeping Top',
    description: 'Spinning perfectly upright. Stability is maintained as long as the spin rate is high enough. Small perturbations recover.',
    params: { startOmegaPsi: 200, startOmegaPhi: 0, tiltAngle: 0.01 },
  },
  {
    name: 'Precession from Push',
    description: 'Give the gyroscope an initial push in the precession direction to eliminate initial dropping and attain steady precession immediately.',
...
export const guidedExperiments = [
  {
    ...
    steps: [
      {
        instruction: 'The wheel starts spinning at 150 rad/s. A strong gravity torque is pulling it down. What does the wheel do?',
        ...
        choices: ['It falls straight down', 'It orbits horizontally (Precesses)', 'It spins backwards'],
        correctIndex: 1,
        commonMisconception: 'Many expect the wheel to just fall. However, the cross product of the gravitational torque with the angular momentum causes a perpendicular precession.',
        explanation: 'The wheel precesses horizontally! To conserve angular momentum in the face of downward torque, the resulting motion is 90 degrees to the applied force.',
      },
      {
        instruction: 'Now lets try a very low spin rate. We reduce spin from 150 to 20 rad/s.',
        params: { startOmegaPsi: 20, startOmegaPhi: 0, tiltAngle: 1.1, damping: 0, spinDecay: 0, trail: 600 },
        question: 'How does the motion change?',
        choices: ['It falls instantly', 'It precesses faster with a deep wobble', 'It precesses slower'],
        correctIndex: 1,
        commonMisconception: 'Most people don\'t notice nutation because fast tops do it imperceptibly small. A slow gyroscope exhibits huge, petal-like bobs called nutation as it struggles to precess.',
        explanation: 'At slow speeds, the wheel drops significantly before building up enough precession speed to "catch" itself, resulting in a cycloid or petal-like bouncing trace (Nutation).',
      },
    ],
  },
];
...
export function create(canvas, initParams = {}) {
  ...
  function initState() {
    ...
    th = p.tiltAngle; ph = 0; ps = 0;
    th_v = p.startOmegaTheta; ph_v = p.startOmegaPhi; ps_v = p.startOmegaPsi;
  ...
  function render() {
    ...
    // Environment grids
    drawLine3({ x: -2.0, y: 0, z: 0 }, { x: 2.0, y: 0, z: 0 }, 'rgba(96,165,250,0.3)', 1, cx, cy, focal);
    drawLine3({ x: 0, y: -2.0, z: 0 }, { x: 0, y: 2.0, z: 0 }, 'rgba(16,185,129,0.3)', 1, cx, cy, focal);
    drawLine3({ x: 0, y: 0, z: 0 }, { x: 0, y: 0, z: 2.0 }, 'rgba(248,113,113,0.3)', 1, cx, cy, focal);
    ...
    for (let j = -1; j <= 1; j += 2) {
      ...
      for (let i = 0; i <= rimSegments; i++) {
          ...
          const pr = project(pt, camera, cx, cy, focal);
          if (pr.z < 0.1) continue;
          if (i === 0) { ctx.moveTo(pr.x, pr.y); firstPoint = pr; }
          else ctx.lineTo(pr.x, pr.y);
      }
    ...
    const angMomTip = { x: center.x + axis.x * 0.35, y: center.y + axis.y * 0.35, z: center.z + axis.z * 0.35 };
  ...
  let rafId, running = false, lastTs, speedScale = 1;
  ...
  return {
    ...
    setParams(next) {
      ...
      if (Math.abs(p.tiltAngle - prevTilt) > 1e-6 || Math.abs(p.startOmegaPsi - prevStartPsi) > 1e-6) {
        th = p.tiltAngle;
