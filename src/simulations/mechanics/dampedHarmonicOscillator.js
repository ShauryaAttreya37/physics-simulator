...
export const equationSections = [
  {
    title: 'Introduction',
    content:
      'A damped harmonic oscillator is a mass on a spring with friction (damping). Without damping, it oscillates forever. With damping, it slows down and stops. This models many real systems like car suspensions, guitar strings, or pendulum clocks. The damping ratio determines if it oscillates or not.',
  },
  {
    ...
    equations: [
      {
        ...
        description:
          "Newton's law: mass times acceleration equals the forces. Spring pulls back (kx), damping resists motion (c times velocity), no external forces.",
      },
      {
        latex: String.raw`\ddot{x} + 2\zeta\omega_n\dot{x} + \omega_n^2 x = 0, \quad \omega_n = \sqrt{k/m}, \quad \zeta = \frac{c}{2\sqrt{mk}}`,
        description:
          'Normalized form. ωₙ is the natural frequency (how fast it would oscillate without damping). ζ (zeta) is damping ratio - less than 1 oscillates, equal 1 critical, greater 1 no oscillation.',
      },
    ],
    variables: [
    ...
    equations: [
      {
        ...
        description:
          'Underdamped case: oscillates while decaying. The amplitude decreases exponentially, frequency ωd is less than natural frequency.',
      },
      {
        latex: String.raw`\zeta = 1: \quad x(t) = (A + Bt)e^{-\omega_n t}`,
        description:
          'Critical damping: returns to equilibrium as fast as possible without oscillating. Used in doors that close smoothly.',
      },
      {
        latex: String.raw`\zeta > 1: \quad x(t) = A e^{r_1 t} + B e^{r_2 t}, \quad r_{1,2} = -\zeta\omega_n \pm \omega_n\sqrt{\zeta^2-1}`,
        description:
          'Overdamped: slow decay without oscillation. Like a heavy door that closes very slowly.',
      },
    ],
  },
  {
    ...
    equations: [
      {
        ...
        description:
          'Total energy decreases over time due to damping. The rate of energy loss is proportional to velocity squared. This is why damped systems eventually stop.',
      },
    ],
  },
  {
    title: 'How to Use',
    content:
      '1. Adjust mass m and spring constant k to change natural frequency.\n2. Increase damping c to see different behaviors.\n3. Try ζ < 1 for oscillation, ζ = 1 for critical, ζ > 1 for no oscillation.\n4. Set initial displacement and velocity.\n5. Watch position, velocity, and energy graphs over time.',
  },
  {
    title: 'Beginner Tips',
    content:
      'Start with low damping to see clear oscillations. Increase damping gradually and watch the transition from underdamped to overdamped. Notice energy decreases due to damping. Try different initial conditions - velocity affects the phase. Look at the phase space plot (position vs velocity).',
  },
];
...
export const scenarios = [
  ...
  },
  {
    ...
    description:
      'Just barely underdamped — the system completes less than one oscillation before settling.',
    ...
    steps: [
      {
        ...
        choices: [
          'The mass will oscillate forever',
          'It will slow down and stop',
          'It will oscillate faster and faster',
        ],
        correctIndex: 0,
        explanation:
          'With zero damping, total mechanical energy E = ½mv² + ½kx² is conserved. The system is a perfect simple harmonic oscillator. In the Phase tab, the trajectory draws a perfect ellipse.',
      },
      {
        ...
        choices: [
          'Peaks stay the same height',
          'Each peak is slightly lower (exponential decay)',
          'Peaks increase then decrease',
        ],
        correctIndex: 1,
        explanation:
          "This is the underdamped regime (ζ < 1). The amplitude decays as e^(-ζωₙt). Watch the yellow dashed envelope — that's the exponential decay curve.",
        tryThis:
          'Look at the energy readout: E is decreasing because damping dissipates energy at rate dE/dt = -cv².',
      },
      {
        instruction:
          'Set damping to c = 12.649 (critical damping for these parameters). Reset and play.',
        params: { mass: 1.0, springK: 40, damping: 12.649, x0: 2.0, v0: 0 },
        question: 'At exactly critical damping, will the mass overshoot x = 0?',
        choices: [
          'Yes, it will overshoot once',
          'No — it returns to zero without overshooting',
          'It will oscillate exactly once',
        ],
        correctIndex: 1,
        commonMisconception:
          'Students think "more damping = faster settling." Critical damping (ζ = 1) is actually the sweet spot — the fastest return without oscillation. Overdamping is slower.',
        explanation:
          'At ζ = 1, the solution is x(t) = (A + Bt)e^(-ωₙt). No oscillation, no overshoot. This is why car shock absorbers are tuned to near-critical damping — you want fast settling without bouncing.',
        tryThis:
          "Now increase to c = 25 (overdamped). Notice it settles MORE SLOWLY than critical — that's counterintuitive!",
      },
    ],
  },
];
  ...
  function render() {
    ...
    if (trail.length >= 2) {
      ...
      if (tMax > tMin) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.moveTo(gx + gw - 100, gy + 12);
        ctx.lineTo(gx + gw - 80, gy + 12);
        ctx.stroke();
        ctx.fillStyle = '#60a5fa';
        ctx.fillText('RK4', gx + gw - 76, gy + 15);
        ctx.setLineDash([4, 4]);
        ctx.strokeStyle = 'rgba(255,255,255,0.3)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(gx + gw - 100, gy + 26);
        ctx.lineTo(gx + gw - 80, gy + 26);
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = 'rgba(255,255,255,0.4)';
        ctx.fillText('Exact', gx + gw - 76, gy + 29);
      }
      ...
      if (phh > 50) {
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(gx, py0 + phh);
        ctx.lineTo(gx + pww, py0 + phh);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(gx, py0);
        ctx.lineTo(gx, py0 + phh);
        ctx.stroke();
