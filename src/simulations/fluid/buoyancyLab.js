...
export function create(canvas, initParams = {}) {
  ...
  canvas.addEventListener('pointerdown', e => {
    ...
    if (hit) {
      ...
      if (blocks.length < 10) {
        const newBlock = { id: Date.now()+Math.random(), mat, x: Math.max(TK_X+20, Math.min(sx, TK_X+TK_W-20)), y: Math.max(TK_Y+20, sy), vy: 0 };
  ...
  canvas.addEventListener('pointermove', e => {
    ...
    const hw = dragging.block.mat.w / 2, hh = dragging.block.mat.h / 2;
  ...
  const stopDrag = () => { if(dragging) { dragging.block.dragging = false; dragging = null; } };
    ...
    for (const b of blocks) {
      ...
      if (subFrac > 0.05 && Math.abs(oldVy) > 40) {
        ...
        for (let i = 0; i < Math.min(nBub, 3); i++) {
          bubbles.push({
            x: b.x + (Math.random() - 0.5) * b.mat.w * 0.8, y: fTop + sub * Math.random(),
            r: 1.5 + Math.random() * 2.5, vy: -20 - Math.random() * 40, life: 1.0,
          });
        }
      }
      ...
      for (const other of blocks) {
        if (other === b) continue;
        const dx = Math.abs(b.x - other.x), dy = Math.abs(b.y - other.y);
        const overlapX = (b.mat.w + other.mat.w) / 2 - dx, overlapY = (b.mat.h + other.mat.h) / 2 - dy;
  ...
  function bDrawBlock(b, fl, isSelected, fTop) {
    ...
    const dryH = mat.h * (1 - subFrac), wetH = mat.h * subFrac;
    const r = 4;
    ...
    if (dryH > 0) {
      ctx.save(); ctx.beginPath(); roundRect(ctx, bLeft, bTop, mat.w, mat.h, r); ctx.clip();
      const g1 = ctx.createLinearGradient(bLeft, bTop, bLeft + mat.w, bTop + dryH);
      g1.addColorStop(0, lighten(mat.color, 0.22)); g1.addColorStop(0.5, mat.color); g1.addColorStop(1, lighten(mat.color, -0.05));
      ctx.fillStyle = g1; ctx.fillRect(bLeft, bTop, mat.w, dryH);
      ctx.fillStyle = 'rgba(255,255,255,0.12)'; ctx.fillRect(bLeft, bTop, mat.w, 2);
      ctx.fillStyle = '#2A3441'; ctx.fillRect(bLeft, bTop, 2, dryH);
      ctx.restore();
    }
    ...
    if (wetH > 0) {
      ...
      g2.addColorStop(0, blend(mat.color, fl.color, 0.30)); g2.addColorStop(0.5, blend(mat.color, fl.color, 0.42)); g2.addColorStop(1, blend(mat.color, fl.color, 0.55));
      ctx.fillStyle = g2; ctx.fillRect(bLeft, wetTop, mat.w, wetH);
      ctx.fillStyle = hexAlpha(fl.colorTop, 0.10); ctx.fillRect(bLeft, wetTop, mat.w, wetH);
      ctx.restore();
    }

    ctx.beginPath(); roundRect(ctx, bLeft, bTop, mat.w, mat.h, r);
    if (isSelected) {
      ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 2; ctx.shadowBlur = 12; ctx.shadowColor = 'rgba(255,255,255,0.4)';
    } else {
      ctx.strokeStyle = mat.stroke; ctx.lineWidth = 1.5;
    }
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(mat.name, x, y - 1);
  ...
  function bDrawScale(sel, fl, fTop) {
    ...
    ctx.fillStyle = '#1a2538'; roundRect(ctx, sx - 40, mountY - 14, 80, 16, 4); ctx.fill();
    ctx.strokeStyle = '#2d4060'; ctx.lineWidth = 1; roundRect(ctx, sx - 40, mountY - 14, 80, 16, 4); ctx.stroke();
    ctx.fillStyle = '#3a5070'; ctx.fillRect(sx - 3, mountY + 2, 6, 22);
    ...
    for (let i = 0; i <= coils * 2; i++) {
        const t = i / (coils * 2); const side = (i % 2 === 0 ? 1 : -1) * 10;
        ctx.lineTo(sx + side, hookY + springLen * t);
    }
    ctx.lineTo(sx, panY);
    ctx.strokeStyle = '#60a5fa'; ctx.lineWidth = 2; ctx.shadowBlur = 6; ctx.shadowColor = '#3b82f680';
    ctx.stroke(); ctx.shadowBlur = 0;

    ctx.beginPath(); ctx.moveTo(sx - 26, panY); ctx.lineTo(sx + 26, panY); ctx.strokeStyle = '#8899aa'; ctx.lineWidth = 3; ctx.stroke();
    ctx.beginPath(); ctx.moveTo(sx - 26, panY); ctx.lineTo(sx - 20, panY + 5); ctx.lineTo(sx + 20, panY + 5); ctx.lineTo(sx + 26, panY);
    ctx.strokeStyle = '#556677'; ctx.lineWidth = 1; ctx.stroke();

    if (sel) {
      const sc = 0.5, mw = sel.mat.w * sc, mh = sel.mat.h * sc;
      roundRect(ctx, sx - mw / 2, panY - mh + 1, mw, mh, 3);
      ctx.fillStyle = sel.mat.color; ctx.fill();
      ctx.strokeStyle = sel.mat.stroke; ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = 'rgba(255,255,255,0.65)'; ctx.font = 'bold 7px "Montserrat", sans-serif';
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(sel.mat.name, sx, panY - mh / 2 + 1);
    }

    const boxY = panY + 16, boxW = 136, boxH = 130;
    ctx.fillStyle = 'rgba(8,14,26,0.94)'; roundRect(ctx, sx - boxW / 2, boxY, boxW, boxH, 8); ctx.fill();
    ctx.strokeStyle = '#1a3050'; ctx.lineWidth = 1; roundRect(ctx, sx - boxW / 2, boxY, boxW, boxH, 8); ctx.stroke();
    
    ctx.textAlign = 'center';
    if (sel) {
      const subFrac = computeSubFrac(sel, fTop);
      const wt = bWeight(sel.mat), buoyF = bBuoy(sel.mat, fl, subFrac), appWt = bApparent(sel.mat, fl, subFrac);
      const floats = appWt < -0.00005, neutral = Math.abs(appWt) <= 0.00005;

      ctx.fillStyle = '#5a6a7a'; ctx.font = '9px "Montserrat", sans-serif'; ctx.fillText('Weight (in air)', sx, boxY + 16);
      ctx.fillStyle = '#f1f5f9'; ctx.font = 'bold 12px "JetBrains Mono", monospace'; ctx.fillText(`${wt.toFixed(4)} N`, sx, boxY + 30);
      ctx.fillStyle = '#5a6a7a'; ctx.font = '9px "Montserrat", sans-serif'; ctx.fillText('Buoyant force', sx, boxY + 46);
      ctx.fillStyle = '#60a5fa'; ctx.font = 'bold 12px "JetBrains Mono", monospace'; ctx.fillText(`${buoyF.toFixed(4)} N`, sx, boxY + 60);
      ctx.fillStyle = '#5a6a7a'; ctx.font = '9px "Montserrat", sans-serif'; ctx.fillText('Apparent weight', sx, boxY + 76);
      ctx.fillStyle = floats ? '#86efac' : neutral ? '#FFD166' : '#fca5a5'; ctx.font = 'bold 13px "JetBrains Mono", monospace';
      ...
      roundRect(ctx, sx - 36, boxY + 102, 72, 18, 4); ctx.fillStyle = tagBg; ctx.fill();
      ctx.strokeStyle = floats ? 'rgba(134,239,172,0.25)' : neutral ? 'rgba(255, 209, 102,0.25)' : 'rgba(248,113,113,0.25)';
      ctx.lineWidth = 1; ctx.stroke();
      ctx.fillStyle = tagCol; ctx.font = 'bold 9px "Montserrat", sans-serif'; ctx.fillText(tag, sx, boxY + 114);
    } else {
    ...
    ctx.fillStyle = 'rgba(148,163,184,0.4)'; ctx.font = '9px "Montserrat", sans-serif'; ctx.textAlign = 'center';
  ...
  function render() {
    ...
    bgGrad.addColorStop(0, '#0a0c16'); bgGrad.addColorStop(1, '#050610');
    ...
    tankGrad.addColorStop(0, 'rgba(8,14,28,0.95)'); tankGrad.addColorStop(1, 'rgba(4,8,18,0.98)');
    ctx.fillStyle = tankGrad; ctx.fillRect(TK_X, TK_Y, TK_W, TK_H);
    ctx.fillStyle = 'rgba(255,255,255,0.015)'; ctx.fillRect(TK_X, TK_Y, 3, TK_H); ctx.fillRect(TK_X + TK_W - 3, TK_Y, 3, TK_H);
    ctx.restore();

    ctx.strokeStyle = '#2a3650'; ctx.lineWidth = 2.5; ctx.strokeRect(TK_X, TK_Y, TK_W, TK_H);
    ctx.strokeStyle = 'rgba(100,140,200,0.08)'; ctx.lineWidth = 1; ctx.strokeRect(TK_X + 1, TK_Y + 1, TK_W - 2, TK_H - 2);

    const fTop = dynamicFluidTopPx(), fH = TK_Y + TK_H - fTop;
    ...
    ctx.fillStyle = fGrad; ctx.fillRect(TK_X + 2, fTop, TK_W - 4, fH);
    ...
    for (let i = 0; i < 6; i++) {
        ...
        cGrad.addColorStop(0, 'rgba(255,255,255,0.8)'); cGrad.addColorStop(1, 'rgba(255,255,255,0)');
        ctx.fillStyle = cGrad; ctx.fillRect(rx, fTop, rw, fH * 0.7);
    }
    ctx.globalAlpha = 1;
    ctx.restore();

    ctx.save();
    ctx.beginPath(); ctx.moveTo(TK_X + 2, fTop);
    for (let px = TK_X + 2; px <= TK_X + TK_W - 2; px += 2) {
        const wave = Math.sin((px - TK_X) / TK_W * Math.PI * 6 + time * 3) * 1.2 + Math.sin((px - TK_X) / TK_W * Math.PI * 10 + time * 5) * 0.5;
        ctx.lineTo(px, fTop + wave);
    }
    ctx.lineTo(TK_X + TK_W - 2, fTop + 8); ctx.lineTo(TK_X + 2, fTop + 8); ctx.closePath();
    const surfGrad = ctx.createLinearGradient(0, fTop - 2, 0, fTop + 8);
    surfGrad.addColorStop(0, hexAlpha(fl.colorTop, 0.95)); surfGrad.addColorStop(1, hexAlpha(fl.color, 0.3));
    ...
    ctx.strokeStyle = hexAlpha(fl.colorTop, 0.9); ctx.lineWidth = 1.5; ctx.shadowBlur = 4; ctx.shadowColor = hexAlpha(fl.colorTop, 0.4);
    ctx.stroke(); ctx.shadowBlur = 0;

    for (const bub of bubbles) {
      ctx.beginPath(); ctx.arc(bub.x, bub.y, bub.r, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(180,220,255,${bub.life * 0.3})`; ctx.fill();
      ctx.strokeStyle = `rgba(200,235,255,${bub.life * 0.25})`; ctx.lineWidth = 0.5; ctx.stroke();
    }

    for (const b of blocks) { if (b.id !== selectedId) bDrawBlock(b, fl, false, fTop); }
    const sel = blocks.find(b => b.id === selectedId);
    if (sel) bDrawBlock(sel, fl, true, fTop);

    if (sel) {
      const subFrac = computeSubFrac(sel, fTop), wt = bWeight(sel.mat), buoyF = bBuoy(sel.mat, fl, subFrac);
      const maxF = Math.max(wt, buoyF, 0.001), arrowScale = 60, wtLen = (wt / maxF) * arrowScale;
      const bCx = sel.x, bCy = sel.y + sel.mat.h / 2;
      
      ctx.beginPath(); ctx.moveTo(bCx - 12, bCy + 2); ctx.lineTo(bCx - 12, bCy + 2 + wtLen);
      ctx.strokeStyle = '#FF6B6B'; ctx.lineWidth = 2.5; ctx.stroke();
      ctx.beginPath(); ctx.moveTo(bCx - 12, bCy + 2 + wtLen); ctx.lineTo(bCx - 16, bCy + 2 + wtLen - 6); ctx.lineTo(bCx - 8, bCy + 2 + wtLen - 6); ctx.closePath();
      ctx.fillStyle = '#FF6B6B'; ctx.fill();

      if (subFrac > 0.001) {
        const buoyLen = (buoyF / maxF) * arrowScale;
        ctx.beginPath(); ctx.moveTo(bCx + 12, bCy + 2); ctx.lineTo(bCx + 12, bCy + 2 - buoyLen);
        ctx.strokeStyle = '#60a5fa'; ctx.lineWidth = 2.5; ctx.stroke();
        ctx.beginPath(); ctx.moveTo(bCx + 12, bCy + 2 - buoyLen); ctx.lineTo(bCx + 8, bCy + 2 - buoyLen + 6); ctx.lineTo(bCx + 16, bCy + 2 - buoyLen + 6); ctx.closePath();
        ctx.fillStyle = '#60a5fa'; ctx.fill();
      }
    }

    bDrawScale(sel, fl, fTop);
    ctx.fillStyle = 'rgba(255,255,255,0.14)'; ctx.font = 'bold 11px "Montserrat", sans-serif';
    ctx.textAlign = 'center'; ctx.letterSpacing = '0.1em'; ctx.fillText('ARCHIMEDES\' BUOYANCY TANK', TK_X + TK_W / 2, TK_Y - 10);
  }

  let rafId, running = false, lastTs;
  ...
  return {
    start() { running = true; lastTs = performance.now(); loop(lastTs); },
    stop() { running = false; cancelAnimationFrame(rafId); },
    reset() { this.stop(); blocks = []; bubbles = []; time = 0; selectedId = null; render(); this.start(); },
    destroy() { this.stop(); },
    setParams(newP) { p = { ...p, ...newP }; },
    getData() {
      const fl = FLUIDS[p.fluidIdx];
      const sel = blocks.find(b => b.id === selectedId);
      if (!sel) return {};
