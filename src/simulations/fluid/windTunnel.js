...
const SHAPES = [
  { id:'circle',  name:'Cylinder',   cd:1.20, cl:0,    st:0.20 },
  { id:'box',     name:'Flat Plate', cd:1.98, cl:0,    st:0.15 },
  { id:'sphere3d',name:'Sphere 3D',  cd:0.47, cl:0,    st:0.19 },
  { id:'airfoil', name:'Airfoil',    cd:0.008,cl:0.30, st:0.0  },
  { id:'diamond', name:'Diamond',    cd:1.50, cl:0,    st:0.18 },
];
...
function cdForRe(shape, Re) {
  ...
  if (shape.id === 'circle') {
    ...
    if (Re < 6e5)  return 1.20 - 0.90 * ((Re-3e5)/3e5);
    return 0.30;
  }
  if (shape.id === 'airfoil') {
...
export function create(canvas, initParams = {}) {
  ...
  let TN_TOP = 45, TN_BOT = 405, TN_LEFT = 20, TN_RIGHT = canvas.width - 20;
  let OBJ_CX = canvas.width * 0.4, OBJ_CY = canvas.height * 0.5;
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    mouseProbe = { x: (e.clientX - rect.left)*scaleX, y: (e.clientY - rect.top)*scaleY, active: true };
  });
  ...
  function drawTunnelShape(ctx, shape, cx, cy) {
    ...
    if (shape.id === 'circle' || shape.id === 'sphere3d') {
      ...
      if (shape.id === 'sphere3d') {
        ...
        ctx.strokeStyle = '#3a5070'; ctx.lineWidth = 1.5; ctx.stroke();
      ...
      ctx.strokeStyle = '#6060a0'; ctx.lineWidth = 1.5; ctx.strokeRect(-w/2, -h/2, w, h);
  ...
  function tick(dt) {
    ...
    for (const pt of particles) {
      ...
      const dx = pt.x - OBJ_CX, dy = pt.y - OBJ_CY;
      const reset = pt.x > TN_RIGHT + 20 || pt.x < TN_LEFT - 40
                 || pt.y < TN_TOP - 5    || pt.y > TN_BOT + 5
  ...
  function render() {
    ...
    for (let y = TN_TOP + 20; y < TN_BOT; y += 38) {
      ...
      ctx.beginPath(); ctx.moveTo(TN_LEFT + 5, y); ctx.lineTo(TN_LEFT + 5 + arrowLen, y);
      ctx.strokeStyle = `rgba(96,165,250,${0.3 + speed * 0.3})`;
      ctx.lineWidth = 1; ctx.stroke();
      ctx.beginPath(); ctx.moveTo(TN_LEFT + 5 + arrowLen, y);
      ctx.lineTo(TN_LEFT + 5 + arrowLen - 5, y - 3); ctx.lineTo(TN_LEFT + 5 + arrowLen - 5, y + 3);
      ctx.closePath(); ctx.fillStyle = `rgba(96,165,250,${0.3 + speed * 0.3})`; ctx.fill();
    }
    ...
    for (const pt of particles) {
      ...
      if (pt.trail.length > 1) {
        ctx.beginPath(); ctx.moveTo(pt.trail[0].x, pt.trail[0].y);
        ...
        grad.addColorStop(0, `rgba(${r},${g},${b},0.0)`); grad.addColorStop(1, `rgba(${r},${g},${b},0.6)`);
        ctx.strokeStyle = grad; ctx.lineWidth = 1.5; ctx.stroke();
      }
      ctx.beginPath(); ctx.arc(pt.x, pt.y, 1.8, 0, Math.PI*2);
      ctx.fillStyle = `rgba(255,255,255,0.9)`;
      ctx.shadowBlur = 8; ctx.shadowColor = `rgba(${r},${g},${b},1)`;
    ...
    if (mouseProbe && mouseProbe.active) {
      ...
      ctx.beginPath(); ctx.arc(mouseProbe.x, mouseProbe.y, 4, 0, Math.PI*2);
  ...
  return {
    ...
    reset() { this.stop(); particles = []; simTime = 0; render(); this.start(); },
    destroy() { this.stop(); },
    setParams(newP) {
