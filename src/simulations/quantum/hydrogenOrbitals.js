...
function calcPsiReal(n, l, m, x, y, z) {
  ...
  if (n === 1 && l === 0) R *= 1; // 1s
  else if (n === 2 && l === 0) R *= (2 - p); // 2s
  else if (n === 2 && l === 1) R *= 1; // 2p
  else if (n === 3 && l === 0) R *= (27 - 18*p + 2*p*p); // 3s
  else if (n === 3 && l === 1) R *= (6 - p); // 3p
  else if (n === 3 && l === 2) R *= 1; // 3d
  else if (n === 4 && l === 0) R *= (192 - 144*p + 24*p*p - p*p*p); // 4s
  else if (n === 4 && l === 1) R *= (80 - 20*p + p*p); // 4p
  else if (n === 4 && l === 2) R *= (8 - p); // 4d
  else if (n === 4 && l === 3) R *= 1; // 4f
  else R *= 1; // fallback
  ...
  if (l === 1) { // P orbitals
    ...
    if (m === 0) Y = 3*z*z - r2; // d_z2
    else if (m === 1) Y = x*z; // d_xz
    else if (m === -1) Y = y*z; // d_yz
    else if (m === 2) Y = x*x - y*y; // d_x2-y2
    else if (m === -2) Y = x*y; // d_xy
  } else if (l === 3) { // F orbitals
    if (m === 0) Y = z * (5*z*z - 3*r2); // f_z3
    else if (m === 1) Y = x * (5*z*z - r2); // f_xz2
    else if (m === -1) Y = y * (5*z*z - r2); // f_yz2
    else if (m === 2) Y = z * (x*x - y*y); // f_z(x2-y2)
    else if (m === -2) Y = x*y*z; // f_xyz
    else if (m === 3) Y = x * (x*x - 3*y*y); // f_x(x2-3y2)
...
export function create(canvas, initParams = {}) {
  ...
  // Cloud cache
  let pointCloud = [];
  let cachedN = -1, cachedL = -1, cachedM = -1;
  ...
  function getOrbitalSymbol(n, l, m) {
    ...
    let sub = '';
    if (l === 1) {
      if (m === 0) sub = 'z'; if (m === 1) sub = 'x'; if (m === -1) sub = 'y';
    } else if (l === 2) {
      if (m === 0) sub = 'z²'; if (m === 1) sub = 'xz'; if (m === -1) sub = 'yz';
      if (m === 2) sub = 'x²-y²'; if (m === -2) sub = 'xy';
    } else if (l === 3) {
  ...
  function enforceQuantumRules() {
    ...
    if (p.n !== cachedN || L !== cachedL || M !== cachedM) {
      cachedN = p.n; cachedL = L; cachedM = M;
  ...
  return {
    ...
    setParams(next) { Object.assign(p, next); enforceQuantumRules(); },
    destroy() { 
      this.stop(); 
