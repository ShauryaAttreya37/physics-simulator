...
export function drawMakieGraph(
  canvas,
  {
    data = [],
    series = [],
  ...
  if (phaseSpace && series.length >= 2) {
    ...
    if (data.length > 10) {
      for (let i = 1; i < data.length; i++) {
        const xv0 = data[i - 1][series[0].key],
          yv0 = data[i - 1][series[1].key];
        const xv1 = data[i][series[0].key],
          yv1 = data[i][series[1].key];
        if (
          !Number.isFinite(xv0) ||
          !Number.isFinite(yv0) ||
          !Number.isFinite(xv1) ||
