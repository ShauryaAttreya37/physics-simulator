...
function SliderRow({ label, value, min, max, step = 0.01, onChange, unit }) {
  ...
  return (
    <div className="form-group">
      <label>
        {label}
        <span className="value-pill">{display}{unit ? ` ${unit}` : ''}</span>
      </label>
    ...
    <aside className="inspector-panel ui-panel">
      ...
      <div className="panel-content">
        ...
        <div className="property-section">
          ...
          <SliderRow label="Gravity Y" value={gravity.y} min={-5} max={10} step={0.1}
            onChange={v => setGravity(gravity.x, v)} />
          <SliderRow label="Gravity X" value={gravity.x} min={-5} max={5} step={0.1}
        ...
        {selectedBody && (() => {
          ...
          return (
            <div className="property-section">
              ...
              <div className="form-group">
                <label>Mode</label>
                <select className="form-input" value={props.isStatic ? 'static' : 'dynamic'}
              ...
              {/* Material properties */}
              <div style={{ marginTop: 6, marginBottom: 2, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)' }}>
                Material
              </div>
              <SliderRow label="Restitution" value={props.restitution ?? 0.6}
                min={0} max={1} step={0.01} onChange={v => updateProp('restitution', v)} />
              <SliderRow label="Friction" value={props.friction ?? 0.1}
                min={0} max={1} step={0.01} onChange={v => updateProp('friction', v)} />
              <SliderRow label="Static Friction" value={props.frictionStatic ?? 0.5}
                min={0} max={5} step={0.01} onChange={v => updateProp('frictionStatic', v)} />
              <SliderRow label="Air Drag" value={props.frictionAir ?? 0.01}
                min={0} max={0.5} step={0.001} onChange={v => updateProp('frictionAir', v)} />
              <SliderRow label="Density" value={props.density ?? 0.001}
                min={0.0001} max={0.05} step={0.0001} onChange={v => updateProp('density', v)} />

              {/* Motion */}
              <div style={{ marginTop: 10, marginBottom: 2, fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--text-muted)' }}>
                Motion
              </div>
              <SliderRow label="Vel X" value={props.velocityX ?? 0}
                min={-40} max={40} step={0.5} onChange={v => updateProp('velocityX', v)} unit="px/s" />
              <SliderRow label="Vel Y" value={props.velocityY ?? 0}
                min={-40} max={40} step={0.5} onChange={v => updateProp('velocityY', v)} unit="px/s" />
              <SliderRow label="Angle" value={props.angle ?? 0}
                min={-180} max={180} step={1} onChange={v => updateProp('angle', v)} unit="°" />
              <SliderRow label="Angular Vel" value={props.angularVelocity ?? 0}
                min={-0.5} max={0.5} step={0.01} onChange={v => updateProp('angularVelocity', v)} unit="rad/s" />
              ...
              <button className="btn btn-danger delete-btn" onClick={handleDelete}>Delete</button>
            </div>
          );
        })()}
        ...
        {selectedConstraint && (() => {
          ...
          return (
            <div className="property-section">
              <div className="section-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>{type === 'spring' ? '🔧 Spring' : type === 'string' ? '🧵 String' : type === 'oscillator' ? '📻 Oscillator' : '🔗 Pivot'}</span>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-muted)', fontWeight: 400 }}>
                  {selectedId}
                </span>
              </div>

              <SliderRow label="Stiffness" value={props.stiffness ?? (isSpring ? 0.05 : 1)}
                min={0.001} max={1} step={0.001} onChange={v => updateConProp('stiffness', v)} />
              <SliderRow label="Damping" value={props.damping ?? 0.05}
                min={0} max={1} step={0.01} onChange={v => updateConProp('damping', v)} />
              <SliderRow label="Rest Length" value={props.length ?? 100}
                min={1} max={600} step={1} onChange={v => updateConProp('length', v)} unit="px" />
              ...
              {isSpring && (
                <ReadonlyRow label="Force ≈" value={(Math.abs(stretch) * (props.stiffness ?? 0.05)).toFixed(3)} unit="N" />
              )}
        ...
        {!selectedBody && !selectedConstraint && (
          <div className="empty-state empty-state-spaced">
            ...
            <div style={{ marginTop: 8, fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.7 }}>
              <strong>Click</strong> bodies or springs to select<br />
              <strong>Drag</strong> to move bodies<br />
              <strong>Delete</strong> key to remove selected<br />
              <strong>Alt+Drag</strong> or <strong>Middle-click</strong> to pan<br />
